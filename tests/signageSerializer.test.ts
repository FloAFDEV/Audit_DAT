...
// =================================================================
// Tests de fiabilité du référentiel signalétique.
// Objectif : garantir qu'une base signageReferences enrichie (migrations,
// qualification) survit à l'export, à l'import, au backup avant reset —
// et qu'un vieux fichier d'export (format v1, sans référentiel) ne
// détruit jamais le référentiel local.
// =================================================================
...
describe('Seed et validation', () => {
    it('le seed fournit 44 références valides', async () => {
        const refs = await db.signageReferences.toArray();
        expect(refs).toHaveLength(44);
...
describe('Export complet (format v2)', () => {
    it("exporte une référence modifiée avec TOUS ses champs", async () => {
        const enriched = await enrichAd3();

        const payload = await buildFullExportPayload();

        expect(payload.formatVersion).toBe(2);
        expect(payload.signageReferences).toHaveLength(44);
...
describe('Import / restauration (format v2)', () => {
    it('restaure intégralement une base enrichie après effacement total (scénario reset → restauration)', async () => {
        ...
        expect(await db.signageReferences.count()).toBe(44);
    });
...
describe("Compatibilité anciens exports (format v1) — protection du référentiel local", () => {

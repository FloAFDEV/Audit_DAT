...
import { CheckCircle2, XCircle, Clock, Camera, Trash2, Edit2, Edit, Ban } from 'lucide-react';
...
const PMRFloorAdhesiveAuditForm: React.FC<PMRFloorAdhesiveAuditFormProps> = (props) => {
  ...
  const progress = useMemo(() => {
    if (pmrData.isNotApplicable) return 100;
  ...
  return (
    <>
      ...
      <AuditFormLayout
        ...
        progress={progress}
        onBack={onBack}
        onReset={pmrData.isNotApplicable ? () => {} : onReset}
        ...
        comment={pmrData.isNotApplicable ? pmrData.notApplicableReason : pmrData.comment}
        onCommentChange={pmrData.isNotApplicable ? () => {} : onCommentChange}

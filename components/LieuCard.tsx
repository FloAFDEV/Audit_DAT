...
export const LieuCard: React.FC<LieuCardProps> = ({ lieu, onSelect, activeFilter }) => {
    ...
    const defectCount = auditModeActive ? getLieuDefectCount(lieu) : 0;
    ...
    return (
        <motion.div
            ...
            className={`${cardBgClass} relative p-4 rounded-lg shadow hover:shadow-lg transition-[opacity] duration-200 text-left w-full group flex flex-col h-full dark:ring-1 dark:ring-slate-700/50 dark:hover:ring-slate-600 cursor-pointer ${dimmed ? 'opacity-40' : ''}`}
        >
            {hasAnomaly && (

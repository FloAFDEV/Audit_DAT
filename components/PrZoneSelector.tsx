...
const getZoneIcon = (zone: PrZone) => {
  ...
  if (name.includes('silo'))
    return (
      <div className="p-3 bg-pink-100 text-pink-600 dark:bg-pink-900/40 dark:text-pink-300 rounded-lg">
        <Building {...iconProps} />
      </div>
    );
  ...
  if (name.includes('nord'))
    return (
      <div className="p-3 bg-blue-100 text-blue-600 dark:bg-blue-900/40 dark:text-blue-300 rounded-lg">
        <Navigation2 {...iconProps} style={{ transform: 'rotate(0deg)' }} />
      </div>
    );
  ...
  // Est = à droite (face au nord) → flèche vers la droite
if (name.includes('est'))
  return (
    <div className="p-3 bg-purple-100 text-purple-600 dark:bg-purple-900/40 dark:text-purple-300 rounded-lg">
      <Navigation2 {...iconProps} style={{ transform: 'rotate(90deg)' }} />
    </div>
  );
...
if (name.includes('ouest'))
  ...
  if (name.includes('cov'))
    return (
      <div className="p-3 bg-indigo-100 text-indigo-600 dark:bg-indigo-900/40 dark:text-indigo-300 rounded-lg">
        <Users {...iconProps} />
      </div>
    );
                ...
                <div className="flex justify-between items-center mb-1">
                  <span
                    className={`text-xs font-medium ${
                      isComplete ? 'text-teal-600 dark:text-teal-400' : 'text-gray-500 dark:text-slate-400'
                    }`}
                  >
